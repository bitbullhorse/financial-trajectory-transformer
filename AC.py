...
def train():
    live_time = []
    for episode in count(episodes):
        state = env.reset()
        lam = 1
        done = False
        for t in count():
            action, log_prob = select_action1(state)
            env.render()
            next_state, reward, done, _ = env.step(action)
            next_state_value = critic(torch.from_numpy(next_state).float().unsqueeze(0))
            state_value = critic(torch.from_numpy(state).float().unsqueeze(0))
            ...
            actor_loss = -lam * log_prob * td_error.detach()
            lam *= gamma
