...
# 获取动作空间维度
action_space_dim = env.action_space.shape[0] if hasattr(env.action_space, 'shape') else env.action_space.n
...
class ActorNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.fc = nn.Linear(state_space_dim, 256)
        self.fc1 = nn.Linear(256, 512)
        self.fc2 = nn.Linear(512, 256)
        self.fc3 = nn.Linear(256, 128)
        self.mu_head = nn.Linear(128, 1)
        self.sigma_head = nn.Linear(128, action_space_dim)
...
class Agent:
    ...
    max_grad_norm = 0.5
    ppo_epoch = 10
    buffer_capacity, batch_size = 100000, 128
    ...
    def select_action(self, state):
        ...
        return action.view(-1).cpu().numpy(), action_log_prob.view(-1).numpy()
    ...
    def update(self):
        ...
        old_action_log_probs = torch.tensor([t.a_log_p for t in self.buffer], dtype=torch.float)
        ...
        GAE = []
        gae = 0
        for step in reversed(range(len(r) - 1)):
            delta = r[step] + args.gamma * self.cnet(s[step + 1]) - self.cnet(s[step])
            gae = delta + args.gamma * 0.95 * gae
            GAE.insert(0, gae + self.cnet(s[step]))
        GAE.append(r[len(r) - 1] - self.cnet(s[len(r) - 1]))

        adv = torch.stack(GAE, 0).detach()

        for _ in range(self.ppo_epoch):  # 10
            for index in BatchSampler(
                ...
                gradient = torch.ones_like(action_loss)

                self.optimizer_a.zero_grad()
                action_loss.backward(gradient=gradient)
