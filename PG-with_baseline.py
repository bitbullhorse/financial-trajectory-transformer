...
def loop_episode():
    ...
    for i in range(len(Gt)):
        ...
        V = s_value_func(state_sequence[i]).detach()
        delta = G - V
        delta1 = (G - V).detach()
        ...
        policy_loss_w = -delta.clone()
        policy_loss_w = policy_loss_w * policy_loss_w
        policy_loss_w.backward(retain_graph=True)
        clip_grad_norm_(s_value_func.parameters(), 0.1)
        optimizer_w.step()

        # update policy network
        optimizer_theta.zero_grad()
        policy_loss_theta = -log_act_prob[i] * delta1
        policy_loss_theta.backward(retain_graph=True)
        clip_grad_norm_(policy.parameters(), 0.1)
